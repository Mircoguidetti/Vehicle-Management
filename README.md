...
- ✅ MQTT topic-level access control ready
...
npm install -g mqtt
mqtt pub -h localhost -p 1883 -u admin -P public -t test/topic -m "test message"
```

### TypeORM synchronization issues

If you encounter table conflicts, set `DB_SYNCHRONIZE=false` in `.env` and use migrations:
...
Built with ❤️ using NestJS, TypeScript, MQTT, PostgreSQL, and TimescaleDB
